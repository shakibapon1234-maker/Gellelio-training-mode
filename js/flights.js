(function (global) {
  ...
  global.GalileoFlights = {
    search({ date, origin, destination }) {
      return flights.filter(flight => (!date || flight.date === date) && (!origin || flight.origin === origin) && (!destination || flight.destination === destination));
    },
    byLine(line, results) { return (results || flights).find(flight => flight.line === line) || null; },
    format(flight) {
      return `${String(flight.line).padStart(2, " ")} ${flight.carrier} ${flight.number} ${flight.cls}${flight.seats}  ${flight.origin} ${flight.destination} ${flight.depart} ${flight.arrive}  ${flight.date}`;
    }
  };
}(window));